...
import { useGetProfile } from "@/api/queries"; // ✅ use /profile
import ConnectToSchoolModal from "@/components/ConnectToSchoolModal";

// ⬇️ If your project has a profile store, import it (kept guarded).
...
// Safely pull kids array from common /profile shapes
const extractProfiles = (resp: any): KidProfile[] | null => {
  const root = resp?.data?.data;
  if (!root) return null;
  if (Array.isArray(root)) return root as KidProfile[];
  if (Array.isArray(root?.profiles)) return root.profiles as KidProfile[];
  if (Array.isArray(root?.children)) return root.children as KidProfile[];
  if (Array.isArray(root?.data)) return root.data as KidProfile[];
  return null;
};
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4">
        ...
        <input id={inputId} type="file" accept="image/*" className="hidden" onChange={onPickFile} />
        ...
        <div className="grow">
          ...
          {imageDataUrl ? (
            <button type="button" onClick={onRemovePhoto} className="mt-1 text-xs text-red-600 hover:underline">
              Remove photo
            </button>
          ) : null}
        </div>
...
const MyKids: React.FC = () => {
  ...
  // ✅ Load from /profile (kids list)
  const { data: profileResp, refetch: refetchProfiles } = useGetProfile();
  ...
  // Keep in sync with /profile response when it arrives
  useEffect(() => {
    const kids = extractProfiles(profileResp);
    if (Array.isArray(kids)) {
      setProfiles(kids);
      try { sessionStorage.setItem("profiles", JSON.stringify(kids)); } catch { /* no-op */ }
    }
  }, [profileResp]);
  ...
  // modals
  const [schoolFor, setSchoolFor] = useState<KidProfile | null>(null);     // View School Info
  const [connectFor, setConnectFor] = useState<KidProfile | null>(null);   // Connect to School
  ...
  const persistToSession = (next: KidProfile[]) => {
    try { sessionStorage.setItem("profiles", JSON.stringify(next)); } catch {}
  };
  ...
  const performUpdate = (updated: KidProfile) => {
    ...
    setProfiles(next);
    if (updateProfile) {
      try { updateProfile(updated); } catch {}
    } else {
  ...
  return (
    <div className="space-y-5">
        ...
        {profiles.length === 0 ? (
          ...
          <div className="flex justify-between gap-1 flex-wrap">
            {profiles.map((kid) => {
              ...
              const isConnected = Boolean(kid?.student?.status === "approved");
              const isPending =
                typeof kid?.student?.status === "string" &&
                ["pending", "pending_approval", "awaiting"].includes(
                  kid.student.status.toLowerCase()
                );
              ...
              return (
                <div
                  ...
                  style={{ width: "453px", height: "170px", gap: "10px", opacity: 1 }}
                >
                  <div className="flex items-center gap-4">
                    {/* Avatar */}
                    {img ? (
                  ...
                  <div className="flex items-center mx-auto gap-4">
                    ...
                    {isConnected ? (
                      ...
                      <button
                        onClick={() => setConnectFor(kid)}
                        disabled={isPending}
                        className={classNames(
                          "relative left-4 rounded-full px-4 py-2 text-sm font-semibold shadow",
                          isPending
                            ? "bg-gray-200 text-gray-500 cursor-not-allowed"
                            : "bg-[#9FC43E] text-white hover:brightness-95"
                        )}
                        style={{ lineHeight: "21px", letterSpacing: "0.1px" }}
                      >
                        {isPending ? "Pending approval" : "Connect to School"}
                      </button>
                    )}
                  </div>
                    ...
                    {openMenuFor === kid.id && (
                      <div className="absolute right-0 z-20 mt-2 w-40 overflow-hidden rounded-xl border border-gray-200 bg-white text-sm shadow-lg">
                        ...
                        >
                          ...
                          <span className="font-arimo font-normal text-[14px] leading-[21px] tracking-[0.1px]">
                            Edit Profile
                          </span>
                        </button>
                        <button
      ...
      {/* School Info Modal (connected kids) */}
      <Modal
      ...
      {/* ✅ Connect to School Modal */}
      <ConnectToSchoolModal
        opened={!!connectFor}
        onClose={() => {
          setConnectFor(null);
          // refresh kids when the modal closes (requirement)
          try { refetchProfiles?.(); } catch { /* no-op */ }
        }}
        child={connectFor ? { id: connectFor.id, name: connectFor.name } : null}
        onConnected={() => {
          // refresh immediately after submitting connect (may show as pending)
          try { refetchProfiles?.(); } catch { /* no-op */ }
        }}
      />
