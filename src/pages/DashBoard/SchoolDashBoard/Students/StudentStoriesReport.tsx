...
// -------------------------------
// src/pages/DashBoard/SchoolDashBoard/Students/StudentStoriesReport.tsx
// -------------------------------
import React from "react";
import { useParams, useNavigate, useSearchParams } from "react-router-dom";
import {
...
import { Doughnut, Bar } from "react-chartjs-2";
import { GetLanguageReport, GetStoryReport } from "@/api/api"; // ⬅️ switched endpoints
import storyy from "@/assets/storyy.png";
import useStore from "@/store";
import { getUserState } from "@/store/authStore";
...
type TLangHours = { day: string; hours: number };
type TLanguageReport = {
  watched: number;
  total: number;
  watched_hours: TLangHours[];
  top_contents: Array<{
    category: string;
    title: string;
    status: string;
...
type TStoryHours = { day: string; hours: number };
type TStoryReport = {
  completed_stories: number;
  completed_audios: number;
  quiz_results: TStoryHours[];
  top_contents: Array<{
    category: string;
    title: string;
    status: string;
...
const StudentStoriesReport: React.FC = () => {
  ...
  // role (only used for breadcrumb visibility and resolving profileId)
  const [user] = useStore(getUserState);
  const role = (user?.role || "").toLowerCase();
  const effectiveId = resolveEffectiveId(role, id);
  ...
  // Single period state drives both sections; endpoints don’t take dates (kept for UI).
  const [period, setPeriod] = React.useState<PeriodLabel>("Last 1 week");
  const [loading, setLoading] = React.useState(false);
  const [errorMsg, setErrorMsg] = React.useState<string | null>(null);

  // Data for charts
  const [modeCounts, setModeCounts] = React.useState<{ selfRead: number; audio: number }>({
    selfRead: 0,
    audio: 0,
  });
  const [quizLabels, setQuizLabels] = React.useState<string[]>([]);
  const [quizValues, setQuizValues] = React.useState<number[]>([]);
  const [interestLabels, setInterestLabels] = React.useState<string[]>([]);
  const [interestValues, setInterestValues] = React.useState<number[]>([]);
    ...
    const run = async () => {
      ...
      try {
        ...
        if (selectedCategory === "Languages") {
          const res = await GetLanguageReport(String(effectiveId));
          if (ignore) return;
          const data = (res as any)?.data?.data as TLanguageReport | undefined;

          // Mode
          setModeCounts({
            selfRead: toNum(data?.watched ?? 0),
            audio: 0, // languages endpoint has no audio bucket
          });

          // Quiz/Hours bar
          const wh = data?.watched_hours || [];
          setQuizLabels(wh.map((i) => i.day));
          setQuizValues(wh.map((i) => toNum(i.hours)));

          // Interest
          const tops = data?.top_contents || [];
          setInterestLabels(tops.map((t) => t.title));
          setInterestValues(tops.map((t) => toNum(t.occurrence_count)));
        } else {
          // Stories
          const res = await GetStoryReport(String(effectiveId));
          if (ignore) return;
          const data = (res as any)?.data?.data as TStoryReport | undefined;

          // Mode
          setModeCounts({
            selfRead: toNum(data?.completed_stories ?? 0),
            audio: toNum(data?.completed_audios ?? 0),
          });

          // Quiz/Hours bar
          const qr = data?.quiz_results || [];
          setQuizLabels(qr.map((i) => i.day));
          setQuizValues(qr.map((i) => toNum(i.hours)));
  ...
  }, [effectiveId, selectedCategory]);

  /* ---------- Chart configs (UI intact) ---------- */
  const doughnutData = React.useMemo(
    () => ({
      labels: ["Self Read", "Audio"],
      datasets: [
  ...
  /* ---------- RENDER (exact same layout/classes as your origin) ---------- */
  return (
    <div className="space-y-6 px-4 pb-10">
      {/* breadcrumb */}
      <nav className="text-sm text-gray-600">
